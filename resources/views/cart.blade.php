...
<script>
(function() {
  // Safe PHP → JS string passing via @json() — avoids escaping bugs with apostrophes
  const i18n = {
    yourCart:        @json(__('Your Cart')),
    cartEmpty:       @json(__('Your cart is empty')),
    cartEmptyDesc:   @json(__("Looks like you haven't added anything to your cart yet.")),
    continueShopping:@json(__('Continue Shopping')),
    standard:        @json(__('Standard')),
    clearConfirm:    @json(__('Are you sure you want to clear your cart?')),
  ...
  async function renderCartPage() {
    ...
    if (items.length === 0) {
      if (titleEl) titleEl.textContent = i18n.yourCart;
      container.innerHTML = `
        <div class="empty-cart">
          ...
          <h2>${i18n.cartEmpty}</h2>
          <p>${i18n.cartEmptyDesc}</p>
          <a href="/shop" class="btn-dark" style="display:inline-block; background:#2C1F14; color:#fff; padding:16px 32px; border-radius:12px; text-decoration:none; font-weight:700;">${i18n.continueShopping}</a>
        </div>
      `;
      updateCartSummary([]);
      return;
    ...
    if (titleEl) titleEl.textContent = i18n.yourCart + ' (' + items.length + ')';
      ...
      return `
        <div class="cart-item" data-id="${item.id}">
          ...
          <div class="item-info">
            ...
            <div class="item-meta">${item.variant || i18n.standard}</div>
            <div class="qty-ctrl">
  ...
  function updateCartSummary(items) {
    ...
    if (subtotalLabel) subtotalLabel.textContent = i18n.subtotal + ' (' + items.length + ' ' + i18n.items + ')';
